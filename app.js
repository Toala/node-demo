...
app.get('/voto2',function(req, res){
	res.sendFile(__dirname+'/voteo2.html');
});
...
app.get('/presentaciones/:pre/:diapo', function(req, res){
	var pre=req.params.pre;
	var diapo=req.params.diapo;
  res.sendFile(__dirname + '/presentaciones/'+pre+'/'+diapo);
});

app.get('/presentaciones/:pre/:diapo/:dia', function(req, res){
	var pre=req.params.pre;
	var diapo=req.params.diapo;
	var dia=req.params.dia;
  res.sendFile(__dirname + '/presentaciones/'+pre+'/'+diapo+'/'+dia);
});
...
io.sockets.on('connection', function(socket){
	...
	socket.on('mando voteo', function(data,callback){
		console.log(data.breakout + "hola mundo");	
	...
	socket.on("guardo encuesta",function(data,callback){
		...
		var num=Math.random();
		for (var i = 0,ques=92; i < info.length; i++,ques++) {
	...
	socket.on("guardo encuesta2",function(data,callback){
		...
		//console.log(info);
		var num=Math.random();
		for (var i = 0,ques=32; i < info.length; i++,ques++) {
			var sql="insert into encuesta_respondio(idencuesta_pregunta,nombre,opcion) values(?,?,?)";
	...
	function que_voteo(){
		...
		selecciono(sql,"",function(res){
			socket.emit("que voteo", {data: res},function(callback){});
	...
	function que_experto(){
		...
		var sql="select * from tanger_live";
		selecciono(sql,"",function(res){
